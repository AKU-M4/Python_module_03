...
def process_generator(data):
   ...
   for event in event_generator(data):
      ...
      if total_events < 10:
         print(f"Event {total_events}: Player {player} ({level}) {event_type}")
...
def main()-> None:
   ...
   print(f"Fibonacci sequence (first {len(fib_sequence)}): {fib_sequence}")
